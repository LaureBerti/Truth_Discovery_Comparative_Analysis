...
public class DataQualityLogger {
	public void LogDataSetData(String fileName, DataQualityMeasurments dqm, DataSet dataSet){
		try {
			...
			writer.write("Number of Claims: " + dqm.getTotalNumOfClaims() + "\n");
			writer.write("Gold Standard Values Count: " + dqm.getGoldStandardTrueValueCount()+ "\n");

			writer.write("The number of sources :" + dataSet.getSourcesHash().size()+ "\n");
			writer.write("Average Number Of Claims Per Source: " + dqm.getAverageNumOfClaimsPerSource()+ "\n");

			writer.write("Number of DataItems: " + dataSet.getDataItemClaims().size()+ "\n");
			writer.write("average number of Values Per DataItem: " +  dqm.getAverageNumOfValuesPerDataItem()+ "\n");
			writer.write("Avergae redundency on dataItem: " + dqm.getAverageRedundencyOnDataItem() + "\n");

			writer.write("Precision Of Dominant Value: " + dqm.getPrecisionOfDominantValue()+ "\n");

			writer.write("Number of Objects: " + dqm.getRedundancyOnObjects().size()+ "\n");
			writer.write("Average redundency on objects: " + dqm.getAverageRedundencyOnObject()+ "\n");
	...
	public void logVoterQualityLine(BufferedWriter writer, VoterQualityMeasures measures, boolean header, 
		...
		if (header) {
			...
			line = experiment + " & ";
//			line = line + format.format(measures.getTruePositivePercentage())  + " & ";
